...
const GameCard = ({ game, username, email, sound, clickSound }) => {
    ...
    return (
        <div className=' w-full p-4 rounded-lg shadow-[2px_4px_16px_0px_rgba(0,0,0,0.25)] flex flex-col space-y-2' >
            ...
            <Link onClick={() => sound && clickSound.play()} to={`/play-ground/${username}+${email}+${_id}`}><Button btnType={"primary"}>{status.finished ? "View Game" : "Play!"}</Button></Link>
        </div>
    );
};
